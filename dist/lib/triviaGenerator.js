...
exports.generateTriviaQuestion = generateTriviaQuestion;
const openai_1 = __importDefault(require("openai"));
require("dotenv/config");
const gradeCreativity_1 = require("./gradeCreativity");
const triviaCollection_1 = require("./triviaCollection");
...
async function generateTriviaQuestion(category) {
    const actualCategory = category === "Battle Royale"
        ...
        : category;
    // ✅ Get collection
    const collection = await (0, triviaCollection_1.getTriviaCollection)();
    // 🔍 1. Try loading an unused question from DB
    const existing = await collection.findOneAndUpdate({ category: actualCategory, used: false }, { $set: { used: true } }, { sort: { _id: 1 }, returnDocument: "after" });
    if (existing?.value) {
...
- Include pop culture, clever twists, surprise choices, etc.`,
        ...
        const correctText = parsed.answers?.[["A", "B", "C", "D"].indexOf(parsed.correct?.trim().toUpperCase())];
        if (!questionText || !correctText || !parsed.answers?.length)
            continue;
        // 🔍 Grade creativity
        const creativity = await (0, gradeCreativity_1.gradeCreativity)(questionText, parsed.answers);
        if (creativity < 5) {
        ...
        const newCorrectIndex = shuffled.findIndex((ans) => ans.trim().toLowerCase() === correctText.trim().toLowerCase());
