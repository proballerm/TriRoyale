...
function playerRoom(playerId: string): string {
  return `tournament-player:${playerId}`;
}
...
function scheduleBotAnswer(io: Server, engine: TournamentDuelEngine): void {
  for (const playerId of engine.playerIds) {
    ...
    const answer = correct
      ...
      : question.answers.filter((choice) => choice !== question.correctAnswer)[
    ...
    setTimeout(() => {
      ...
      try {
        ...
        // The question may have advanced while the bot timer was pending.
      }
    }, responseMs);
  }
}
...
function startActiveQuestion(io: Server, engine: TournamentDuelEngine): void {
  ...
  io.to(duelRoom(engine.duelId)).emit("tournamentQuestion", {
    ...publicQuestion(question, engine),
    startTime: startedAt,
  });
  ...
  duelTimers.set(
    engine.duelId,
    setTimeout(() => void finishQuestion(io, engine), question.timeLimit * 1000 + 300),
  );
}
...
async function finishQuestion(io: Server, engine: TournamentDuelEngine): Promise<void> {
  ...
  if (result.nextMatch) {
    ...
    io.to(playerRoom(player.id)).emit("tournamentMatchFound", {
      duel,
      player,
      opponent,
      tournament,
...
export function registerTournamentSocketHandlers(io: Server, socket: Socket): void {
  ...
  socket.on(
    ...
    ({ duelId, questionId, answer }: Record<string, unknown>) => {
      const safeDuelId = sanitizeIdentity(duelId, 100);
